		...
		for (TrkType trk : trkList) { 
			...
			for (TrksegType trkseg : trk.getTrkseg()){ 
				...
				for (WptType trkpt : trkseg.getTrkpt()){
					...
					TrackPoint trackPoint = new TrackPoint(trackPointID,trkpt.getLat().doubleValue(),
							trkpt.getLon().doubleValue(),trkpt.getEle().doubleValue(),
			...
			Track track = new Track(trackID,trk.getName(),segments);
