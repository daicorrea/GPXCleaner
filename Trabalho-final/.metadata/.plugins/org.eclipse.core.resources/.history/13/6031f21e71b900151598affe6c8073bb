...
import unirio.pm.gpx.model.TrackPoint;
 ...
 public class DistanceCalculator {
	...
	static public double distanceBetweenPoints(double x1, double y1, double x2, double y2, double x3, double y3) {		
		...
		//Calculating Slope
		double a = calculateSlope(x1, y1, x3, y3);
		...
		double orthB = calculateOrthogonalYIntercept(orthA, x2, y2);
	...
	static public double calculateSlope(double x1, double y1, double x3, double y3) {
	...
	static public double calculateYIntercept(double a, double x3, double y3) {
	...
	static private double calculateOrthogonalYIntercept(double orthA, double x2, double y2) {
		
		double orthB = y2 + (orthA * x2);
		return orthB;
